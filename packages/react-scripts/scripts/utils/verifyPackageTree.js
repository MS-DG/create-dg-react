...
function verifyReactScriptVersion() {
  ...
  if (requireVersion && semver.compare(ownPackageJson.version, requireVersion) < 0) {
    logger.error(ownPackageJson.name, "was not matched");
    logger.error("required", requireVersion, "but installed", ownPackageJson.version, "was not found");
